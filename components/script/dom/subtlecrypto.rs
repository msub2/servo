...
type EcPublicP256 = elliptic_curve::PublicKey<NistP256>;
type EcPublicP384 = elliptic_curve::PublicKey<NistP384>;
type EcPublicP521 = elliptic_curve::PublicKey<NistP521>;
type EcSecretP256 = elliptic_curve::SecretKey<NistP256>;
type EcSecretP384 = elliptic_curve::SecretKey<NistP384>;
type EcSecretP521 = elliptic_curve::SecretKey<NistP521>;
    ...
    fn ImportKey(
        ...
        let data = match key_data {
            ...
            ArrayBufferViewOrArrayBufferOrJsonWebKey::JsonWebKey(json_web_key) => {
                let kty = match json_web_key.kty {
                    Some(s) => s.to_string(),
                    None => {
                ...
                match kty.as_str() {
                    "oct" => {
                        let Some(k) = json_web_key.k else {
                            promise.reject_error(Error::Syntax);
                            return promise;
                        };
                    ...
                    "EC" => {
                        let Some(crv) = json_web_key.crv else {
                            promise.reject_error(Error::Data);
                            return promise;
                        };
                        let Some(x) = json_web_key.x else {
                            promise.reject_error(Error::Data);
                            return promise;
                        };
                        let Some(y) = json_web_key.y else {
                            promise.reject_error(Error::Data);
                            return promise;
                        };
                        if x.len() != y.len() {
                        ...
                        if let Some(d) = json_web_key.d {
                            if key_usages.iter().any(|usage| {
                                !matches!(usage, &KeyUsage::DeriveBits | &KeyUsage::DeriveKey)
                            }) || key_usages.is_empty()
                            {
                                promise.reject_error(Error::Syntax);
                                return promise;
                            ...
                            match crv.to_string().as_str() {
                                NAMED_CURVE_P256 => {
                                    let Ok(key) = EcPublicP256::from_jwk_str(jwk_string.as_str())
                                    else {
                                        promise.reject_error(Error::Data);
                                        return promise;
                                    };
                                ...
                                NAMED_CURVE_P384 => {
                                    let Ok(key) = EcPublicP384::from_jwk_str(jwk_string.as_str())
                                    else {
                                        promise.reject_error(Error::Data);
                                        return promise;
                                    };
                                ...
                                NAMED_CURVE_P521 => {
                                    let Ok(key) = EcPublicP521::from_jwk_str(jwk_string.as_str())
                                    else {
                                        promise.reject_error(Error::Data);
                                        return promise;
                                    };
...
pub struct SubtleEcdhKeyDeriveParams {
    ...
    pub public_key_handle: Handle,
}

impl SubtleEcdhKeyDeriveParams {
    fn new(params: EcdhKeyDeriveParams) -> Fallible<Self> {
        let params = Self {
            name: params.parent.name.to_string(),
            public_key_type: params.public.Type(),
            public_key_alg: params.public.algorithm(),
            public_key_handle: params.public.handle().clone(),
        };
...
impl SubtleEcKeyImportParams {
    pub fn new(params: EcKeyImportParams) -> Result<Self, Error> {
        let params = SubtleEcKeyImportParams {
            name: params.parent.name.to_string(),
    ...
    pub fn get_key_length(&self) -> Result<u32, Error> {
        match self.named_curve.as_str() {
            NAMED_CURVE_P256 => Ok(32),
            NAMED_CURVE_P384 => Ok(48),
            NAMED_CURVE_P521 => Ok(66),
            _ => Err(Error::NotSupported),
        }
    }
}
    ...
    match algorithm {
        AlgorithmIdentifier::Object(obj) => {
                ...
                name.eq_ignore_ascii_case(ALG_AES_GCM)
            {
                ...
                return Ok(GetKeyLengthAlgorithm::Ecdh(subtle_params));
            } else {
...
fn normalize_algorithm_for_import_key(
    ...
    let name = match algorithm {
        AlgorithmIdentifier::Object(obj) => {
            ...
            } else if name == ALG_ECDH {
                let params = value_from_js_object!(EcKeyImportParams, cx, value);
                let subtle_params = SubtleEcKeyImportParams::new(params)?;
                return Ok(ImportKeyAlgorithm::Ecdh(subtle_params));
            }
...
fn normalize_algorithm_for_derive_bits(
    ...
    let normalized_algorithm = if algorithm.name.str().eq_ignore_ascii_case(ALG_PBKDF2) {
        ...
        let params = value_from_js_object!(EcdhKeyDeriveParams, cx, value);
        let subtle_params = SubtleEcdhKeyDeriveParams::new(params)?;
        DeriveBitsAlgorithm::Ecdh(subtle_params)
    } else {
        return Err(Error::NotSupported);
    };
    ...
    fn import_key_ecdh(
        ...
        match format {
            KeyFormat::Raw => {
                ...
                let key_data = match params.named_curve.as_str() {
                    NAMED_CURVE_P256 => match EcPublicP256::from_sec1_bytes(data) {
                        Ok(key) => key.to_sec1_bytes().to_vec(),
                        Err(_) => return Err(Error::Data),
                    },
                    NAMED_CURVE_P384 => match EcPublicP384::from_sec1_bytes(data) {
                        Ok(key) => key.to_sec1_bytes().to_vec(),
                        Err(_) => return Err(Error::Data),
                    },
                    NAMED_CURVE_P521 => match EcPublicP521::from_sec1_bytes(data) {
                        Ok(key) => key.to_sec1_bytes().to_vec(),
                        Err(_) => return Err(Error::Data),
                    },
                    _ => return Err(Error::Data),
                };
                let handle = Handle::EllipticCurvePublic(key_data, params.named_curve.clone());
                ...
                EcKeyAlgorithm::from_name_and_curve(
                    name.clone(),
                    DOMString::from(params.named_curve.as_str()),
                    algorithm_object.handle_mut(),
                    cx,
                );
            ...
            KeyFormat::Spki => {
                ...
                let spki = SubjectPublicKeyInfo::<AnyRef<'_>, BitString>::from_der(data)
                    .map_err(|_| Error::Data)?;
                let spki_params = spki.algorithm.parameters.ok_or(Error::Data)?;
                let curve = spki_params
                    .decode_as::<EcParameters>()
                    .map_err(|_| Error::Data)?;
                let curve_oid = curve.named_curve().ok_or(Error::Data)?;
                let named_curve = match curve_oid {
                    NistP256::OID => NAMED_CURVE_P256,
                    NistP384::OID => NAMED_CURVE_P384,
                    NistP521::OID => NAMED_CURVE_P521,
                    _ => return Err(Error::Data),
                };
                ...
                let key_bytes = spki
                    ...
                    .ok_or(Error::Data)?
                    .to_vec();
                let handle = Handle::EllipticCurvePublic(key_bytes, params.named_curve.clone());
            ...
            KeyFormat::Pkcs8 => {
                ...
                let pkcs8 = PrivateKeyInfo::from_der(data).map_err(|_| Error::Data)?;
                let pkcs8_params = pkcs8.algorithm.parameters.ok_or(Error::Data)?;
            ...
            KeyFormat::Jwk => {
                ...
                let (key_type, handle) = if usages.is_empty() {
                    match params.named_curve.as_str() {
                        NAMED_CURVE_P256 => {
                            if EcPublicP256::from_sec1_bytes(data).is_err() {
                                return Err(Error::Data);
                            }
                        },
                        NAMED_CURVE_P384 => {
                            if EcPublicP384::from_sec1_bytes(data).is_err() {
                                return Err(Error::Data);
                            }
                        },
                        NAMED_CURVE_P521 => {
                            if EcPublicP521::from_sec1_bytes(data).is_err() {
                                return Err(Error::Data);
                            }
                        },
                        _ => {
    ...
    fn export_key_ecdh(&self, format: KeyFormat, key: &CryptoKey) -> Result<ExportedKey, Error> {
        match format {
            KeyFormat::Raw => {
                ...
                let Handle::EllipticCurvePublic(key_data, _) = key.handle() else {
                    return Err(Error::Data);
                };
            ...
            KeyFormat::Pkcs8 => {
                ...
                let Handle::EllipticCurveSecret(key_data, named_curve) = key.handle() else {
                    return Err(Error::Data);
                };
                let params = match named_curve.as_str() {
                    NAMED_CURVE_P256 => EcParameters::NamedCurve(NistP256::OID),
                    NAMED_CURVE_P384 => EcParameters::NamedCurve(NistP384::OID),
                    NAMED_CURVE_P521 => EcParameters::NamedCurve(NistP521::OID),
                    _ => return Err(Error::Data),
                };
                ...
                let pki = PrivateKeyInfo::new(alg, key_data);
                let der = pki.to_der().map_err(|_| Error::Data)?;
                Ok(ExportedKey::Pkcs8(der))
            },
        }
    }
...
impl SubtleEcdhKeyDeriveParams {
    fn derive_bits(&self, key: &CryptoKey, length: Option<u32>) -> Result<Vec<u8>, Error> {
        // If the [[type]] internal slot of key is not "private", then throw an InvalidAccessError.
        if key.Type() != KeyType::Private {
            return Err(Error::InvalidAccess);
        }
        ...
        let public_curve = public_handle.named_curve().ok_or(Error::Data)?;
        if private_curve != public_curve {
            return Err(Error::InvalidAccess);
        }
...
impl GetKeyLengthAlgorithm {
    fn get_key_length(&self) -> Result<u32, Error> {
        match self {
            ...
            Self::Ecdh(params) => params.get_key_length(),
        }
    }
}
...
impl ImportKeyAlgorithm {
    fn import_key(
        ...
        match self {
            ...
            Self::Ecdh(params) => {
                subtle.import_key_ecdh(format, secret, extractable, key_usages, params)
            },
        }
    }
}
